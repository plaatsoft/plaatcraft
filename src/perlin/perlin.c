...
double perlin_noise(double x, double y, double z) {
    ...
    int A = perlin_p[X  ]+Y, AA = perlin_p[A]+Z, AB = perlin_p[A+1]+Z,      // HASH COORDINATES OF
        B = perlin_p[X+1]+Y, BA = perlin_p[B]+Z, BB = perlin_p[B+1]+Z;      // THE 8 CUBE CORNERS,

    return perlin_lerp(w, perlin_lerp(v, perlin_lerp(u, perlin_grad(perlin_p[AA  ], x  , y  , z   ),  // AND ADD
                     ...
                     perlin_lerp(v, perlin_lerp(u, perlin_grad(perlin_p[AA+1], x  , y  , z-1 ),  // CORNERS
